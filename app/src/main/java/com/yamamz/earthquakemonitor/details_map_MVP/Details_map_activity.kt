...
class Details_map_activity : AppCompatActivity(), OnMapReadyCallback, DetailsMVP.View {
    ...
    override fun getPrefsLat(): String {

        val sharedPrefs = PreferenceManager.getDefaultSharedPreferences(this)
        val locationFromprefs = sharedPrefs.getString("lat", "0.0")
        return  locationFromprefs

    }
    override fun getPrefsLon(): String {

        val sharedPrefs = PreferenceManager.getDefaultSharedPreferences(this)
        val locationFromprefs = sharedPrefs.getString("lon", "0.0")
        return  locationFromprefs
        ...
        // Check that the user hasn't revoked permissions by going to Settings.
                ...
                async(UI) {
                    ...
                    presenter.displayDistance(extras?.getDouble("e").toString().toDouble(),
                            extras?.getDouble("n").toString().toDouble(),getPrefsLat(),getPrefsLon())
                    val manager = this@Details_map_activity.getSystemService(Context.LOCATION_SERVICE) as LocationManager
                    presenter.checkIsProviderEnable(manager.isProviderEnabled(LocationManager.GPS_PROVIDER), hasGPS())
                }
    ...
    private val myReciever = object : BroadcastReceiver() {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (location != null) {
                ...
                editor.putString("lon","${location.longitude}")
                editor.apply()
                Log.e("Yamamz", "${getPrefsLat()} ${getPrefsLon()}")
            }
        }
    }
